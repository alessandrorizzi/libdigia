...
static void horizontalGradientHelper(QPainter *p, const
...
void StyleHelper::horizontalGradient(QPainter *painter, const QRect &spanRect, const QRect &clipRect, bool lightColored)
{
    if (StyleHelper::usePixmapCache()) {
        ...
        if (!QPixmapCache::find(key, pixmap)) {
            ...
            horizontalGradientHelper(&p, rect, lightColored);
            p.end();
            QPixmapCache::insert(key, pixmap);
        }
        ...
        horizontalGradientHelper(painter, clipRect, lightColored);
    }
}
